...
model PDmaxVariable "Discrete Place"
  discrete Integer t(start = startTokens, fixed = true) "marking";
  parameter Integer nInDis(min = 0) = 0 "number of input transitions" annotation(
    Dialog(enable = true, group = "Connector sizing"));
  parameter Integer nOutDis(min = 0) = 0 "number of output transitions" annotation(
    Dialog(enable = true, group = "Connector sizing"));
  parameter Integer nCon(min = 0) = 0 "number of continus transitions" annotation(
    Dialog(enable = true, group = "Connector sizing"));
  parameter Integer nOutExt(min = 0) = 0 "number of output transitions" annotation(
    Dialog(enable = true, group = "Connector sizing"));
  //****MODIFIABLE PARAMETERS AND VARIABLES BEGIN****//
  parameter Integer startTokens = 0 "start tokens" annotation(
    Dialog(enable = true, group = "Tokens"));
  Integer minTokens = 0 "minimum capacity" annotation(
    Dialog(enable = true, group = "Tokens"));
  Integer maxTokens = PNlib.Constants.Integer_inf "maximum capacity" annotation(
    Dialog(enable = true, group = "Tokens"));
  ...
  parameter Integer enablingPrioIn[nInDis] = 1:nInDis "enabling priorities of input transitions" annotation(
    Dialog(enable = if enablingType == PNlib.Types.EnablingType.Priority then true else false, group = "Enabling Dis"));
  parameter Integer enablingPrioOut[nOutDis] = 1:nOutDis "enabling priorities of output transitions" annotation(
    Dialog(enable = if enablingType == PNlib.Types.EnablingType.Priority then true else false, group = "Enabling Dis"));
  parameter Real enablingProbIn[nInDis] = fill(1 / nInDis, nInDis) "enabling probabilities of input transitions" annotation(
    Dialog(enable = if enablingType == PNlib.Types.EnablingType.Probability then true else false, group = "Enabling Dis"));
  parameter Real enablingProbOut[nOutDis] = fill(1 / nOutDis, nOutDis) "enabling probabilities of output transitions" annotation(
    Dialog(enable = if enablingType == PNlib.Types.EnablingType.Probability then true else false, group = "Enabling Dis"));
  parameter Real enablingBeneIn[nInDis] = 1:nInDis "enabling benefit of input transitions" annotation(
    Dialog(enable = if enablingType == PNlib.Types.EnablingType.Benefit then true else false, group = "Enabling Dis"));
  parameter Real enablingBeneOut[nOutDis] = 1:nOutDis "enabling benefit of output transitions" annotation(
    Dialog(enable = if enablingType == PNlib.Types.EnablingType.Benefit then true else false, group = "Enabling Dis"));
  parameter Integer enablingPrioCon[nCon] = 1:nCon "enabling priorities of continus transitions" annotation(
    Dialog(enable = if enablingType == PNlib.Types.EnablingType.Priority then true else false, group = "Enabling Con"));
...
protected
  ...
  //****BLOCKS END****//
public
  PNlib.PN.Interfaces.DisPlaceIn inTransitionDis[nInDis](each tint = pret, each maxTokensint = maxTokens, enable = enableIn.value, fire = fireInDis, arcWeightint = arcWeightInDis, active = activeInDis, enabledByInPlaces = enabledByInPlaces) if nInDis > 0 "connector for input transitions" annotation(
    Placement(transformation(extent = {{-114, -10}, {-98, 10}}, rotation = 0), iconTransformation(extent = {{-116, -10}, {-100, 10}})));
  PNlib.PN.Interfaces.DisPlaceOut outTransitionDis[nOutDis](each tint = pret, each minTokensint = minTokens, enable = enableOut.value, each tokenInOut = pre(tokeninout.value), fire = fireOutDis, arcWeightint = arcWeightOutDis, active = activeOutDis) if nOutDis > 0 "connector for output transitions" annotation(
    Placement(transformation(extent = {{100, -10}, {116, 10}}, rotation = 0)));
  PNlib.PN.Interfaces.HybPlaceOutIn inOutTransitionCon[nCon](each t = pret, each minTokens = minTokens, each maxTokens = maxTokens, enable = enableCon.value, arcWeight = arcWeightCon, active = activeCon) if nCon > 0 "connector for output transitions" annotation(
    Placement(transformation(extent = {{68, -82}, {84, -62}}, rotation = -45)));
  PNlib.PN.Interfaces.PlaceOutExt extOut[nOutExt](each t = pret) if nOutExt > 0 "connector for output extended Arcs" annotation(
    Placement(transformation(extent = {{70, 62}, {86, 82}}, rotation = 45)));
...
//****ERROR MESSENGES END****//
  annotation(
    ...
    Icon(graphics = {Ellipse(extent = {{-100, 96}, {100, -100}}, lineColor = {0, 0, 0}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid), Text(extent = {{-1.5, 25.5}, {-1.5, -21.5}}, lineColor = {0, 0, 0}, textString = DynamicSelect("%startTokens", realString(t, 1, 0))), Text(extent = {{0, -130}, {0, -116}}, lineColor = {0, 0, 0}, textString = DynamicSelect("Cmax=%maxTokens", "Cmax=%maxTokens")), Text(extent = {{0, -150}, {0, -136}}, lineColor = {0, 0, 0}, textString = DynamicSelect("Cmin=%minTokens", "Cmin=%minTokens")), Text(extent = {{-0, 113}, {-0, 138}}, lineColor = {0, 0, 0}, textString = "%name")}),
    Diagram(graphics));
end PDmaxVariable;
