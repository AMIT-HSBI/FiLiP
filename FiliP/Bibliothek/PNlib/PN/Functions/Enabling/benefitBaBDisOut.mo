within PNlib.PN.Functions.Enabling;
function benefitBaBDisOut "Enabling Output Transition by Benefit and Branch and Bound"
...
algorithm
  ...
  // quotient update
  Quotient_[Index]:=-1;
  if not TAout[Index] then
  (TEoutOut,TokenOut ,benefitMaxOut, valid, benefitLimitOut):=benefitBaBDisOut(depth+1, nOut, benefit, arcWeight, Quotient_, t, benefitMaxIn-benefit[Index], minTokens, TEoutOut, TokenOut, benefitLimit, TAout);
  return;
  end if;
  // partial solution 1
  ...
  if depth < nOut then // no finished solution
    if t-TokenOut1 >= minTokens and benefitMaxOut1>=benefitLimit then
        (TEoutOut1,TokenOut1 ,benefitMaxOut1, valid1, benefitLimit):=benefitBaBDisOut(depth+1, nOut, benefit, arcWeight, Quotient_, t, benefitMaxOut1, minTokens, TEoutOut1, TokenOut1, benefitLimit, TAout);
    else
      valid1:=false;
    end if;
    if t-TokenOut2 >= minTokens and benefitMaxOut2>=benefitLimit then
        (TEoutOut2,TokenOut2 ,benefitMaxOut2, valid2 , benefitLimit):=benefitBaBDisOut(depth+1, nOut, benefit, arcWeight, Quotient_, t, benefitMaxOut2, minTokens, TEoutOut2, TokenOut2, benefitLimit, TAout);
    else
      valid2:=false;
    end if;
