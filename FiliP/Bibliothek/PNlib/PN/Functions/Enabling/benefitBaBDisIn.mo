...
function benefitBaBDisIn "Enabling Input Transition by Benefit and Branch and Bound"
  ...
  algorithm
    ...
    // quotient update
    Quotient_[Index]:=-1;
    if not TAein[Index] then
    (TEinOut,TokenOut ,benefitMaxOut, valid, benefitLimitOut):=benefitBaBDisIn(depth+1, nIn, benefit, arcWeight, Quotient_, t, benefitMaxIn-benefit[Index], maxTokens, TEinOut, TokenOut, benefitLimit, TAein);
    return;
    end if;
    // partial solution 1
    TEinOut1[Index] := true;
    ...
    if depth < nIn then // no finished solution
      if t+TokenOut1 <=maxTokens and benefitMaxOut1>=benefitLimit then
          (TEinOut1,TokenOut1 ,benefitMaxOut1, valid1, benefitLimit):=benefitBaBDisIn(depth+1, nIn, benefit, arcWeight, Quotient_, t, benefitMaxOut1, maxTokens, TEinOut1, TokenOut1, benefitLimit, TAein);
      else
        valid1:=false;
      end if;
      if t+TokenOut2 <= maxTokens and benefitMaxOut2>=benefitLimit then
          (TEinOut2,TokenOut2 ,benefitMaxOut2, valid2 , benefitLimit):=benefitBaBDisIn(depth+1, nIn, benefit, arcWeight, Quotient_, t, benefitMaxOut2, maxTokens, TEinOut2, TokenOut2, benefitLimit, TAein);
      else
        valid2:=false;
      end if;
