...
algorithm
  ...
  when delayPassed then
    if nOut>0 then
      ...
      disTAout := PNlib.Functions.OddsAndEnds.boolAnd(TAout, disTransition);
      arcWeightSum := PNlib.Functions.OddsAndEnds.conditionalSum(arcWeight, disTAout);
      //arc weight sum of all active output transitions
      if t - arcWeightSum -minMarks >= -PNlib.Constants.almost_eps or PNlib.Functions.OddsAndEnds.isEqual(arcWeightSum, 0.0) then  //Place has no actual conflict; all active output transitions are enabled
        TEout:=TAout;
      else                          //Place has an actual conflict;
      // hack for Dymola 2017
      // TEout := TAout and not disTransition;
      TEout := PNlib.Functions.OddsAndEnds.boolAnd(TAout, not disTransition);
      if enablingType==PNlib.Types.EnablingType.Priority then     //deterministic enabling according to priorities
        ...
        for i in 1: nOut loop  //discrete transitions are proven at first
          Index:=Modelica.Math.Vectors.find(i,enablingPrio);
           if Index>0 and disTAout[Index] and ((t-arcWeightSum-arcWeight[Index]-minMarks>=-PNlib.Constants.almost_eps) or PNlib.Functions.OddsAndEnds.isEqual(arcWeight[Index], 0.0)) then
             TEout[Index]:=true;
        ...
        for i in 1:nTAout loop
          ...
          if (t-(arcWeightSum + arcWeight[posTE])-minMarks >= -PNlib.Constants.almost_eps) or  PNlib.Functions.OddsAndEnds.isEqual(arcWeight[i], 0.0) then
