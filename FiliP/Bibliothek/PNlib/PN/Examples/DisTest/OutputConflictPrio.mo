...
model OutputConflictPrio
  ...
  PNlib.PN.Components.PD P1(nInDis = 1, startTokens = 2, nOutDis= 2, minTokens = 2) annotation(Placement(transformation(extent={{-30, -10},
            {-10, 10}})));
  ...
  PNlib.PN.Components.PD P3(nInDis = 1) annotation(Placement(transformation(extent={{30, -30}, {50,
            -10}})));
  ...
  inner PNlib.PN.Components.Settings settings annotation(Placement(transformation(extent={{-60, 20},
            {-40, 40}})));
equation
  connect(T3.outPlacesDis[1], P1.inTransitionDis[1]) annotation(Line(points={{-45.2, 0},
          {-30.8, 0}}, color = {0, 0, 0}, smooth = Smooth.None));
  connect(P1.outTransitionDis[1], T1.inPlacesDis[1]) annotation(Line(points={{-9.2, -0.5},
          {0, -0.5}, {0, 20}, {5.2, 20}}, color = {0, 0, 0}, smooth = Smooth.None));
  connect(T1.outPlacesDis[1], P2.inTransitionDis[1]) annotation(Line(points={{14.8, 20},
  ...
  connect(T2.outPlacesDis[1], P3.inTransitionDis[1]) annotation(Line(points={{14.8, -20},
          {29.2, -20}}, color = {0, 0, 0}, smooth = Smooth.None));
