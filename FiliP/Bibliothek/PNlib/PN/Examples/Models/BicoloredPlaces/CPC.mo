...
model CPC
  ...
  PNlib.PN.Interfaces.ConPlaceIn[nIn, numColors] inTransition annotation(Placement(transformation(extent={{-120,-10},{-100,10}}), iconTransformation(extent={{-120,-10},{-100,10}})));
  PNlib.PN.Interfaces.ConPlaceOut[nOut, numColors] outTransition annotation(Placement(transformation(extent={{100,-10},{120,10}}), iconTransformation(extent={{100,-10},{120,10}})));
...
protected
  ...
  PNlib.PN.Components.PC place1(nInCon=nIn, nOutCon=nOut, startTokens=startMarks[1], minTokens=minMarks[1], maxTokens=maxMarks[1]);
  PNlib.PN.Components.PC place2(nInCon=nIn, nOutCon=nOut, startTokens=startMarks[2], minTokens=minMarks[2], maxTokens=maxMarks[2]);
equation
  color = {place1.t, place2.t};

  for i in 1:nIn loop
    connect(inTransition[i,1], place1.inTransitionCon[i]);
    connect(inTransition[i,2], place2.inTransitionCon[i]);
  end for;

  for i in 1:nOut loop
    connect(outTransition[i,1], place1.outTransitionCon[i]);
    connect(outTransition[i,2], place2.outTransitionCon[i]);
  end for;
